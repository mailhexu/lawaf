...
#exit()
downfolder=PhonopyDownfolder(phonopy_yaml=fname, mode="IFC", has_nac=True)
...
lwf=downfolder.downfold(method='scdmk',nwann=3, #selected_basis=[2,5], 
                    anchors={(.0,.0,.0):(0,1, 2)},
                    use_proj=False, mu=-0.25, sigma=9.4, weight_func='Gauss', kmesh=(2,2,2))
...
ax=downfolder.plot_band_fitting(ax=ax, kvectors=np.array([[0. , 0. , 0. ],
           ...
           npoints=200,
    knames=['$\\Gamma$', 'X','M', 'R', 'X', '$\\Gamma$', "R"], plot_original=True, plot_downfolded=False, show=False, fix_LOTO=True,ylabel="eV/$\AA^2$")
